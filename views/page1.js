...
function draw() {
    ...
    let d = dist(point1[0], point1[1], remoteX, remoteY);
    ...
    // --- Círculo propio ---
    drawCircle(point1[0], point1[1], pulsingSize, glowColor);
    ...
    if (d < 50) {
        fill(255);
        noStroke();
        ellipse(point1[0], point1[1], pulsingSize * 1.2);
        ellipse(remoteX, remoteY, pulsingSize * 1.2);

        // Generar nuevas estrellitas
        for (let i = 0; i < 5; i++) {
            stars.push(new Star((point1[0] + remoteX) / 2, (point1[1] + remoteY) / 2));
        }
    }
